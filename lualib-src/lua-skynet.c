...
static int 
lcmdscene(lua_State *L) {
	//struct skynet_context * context = lua_touserdata(L, lua_upvalueindex(1));
    
    int cmd = (int)luaL_checkinteger(L,1);
    int uid ;
    int x ;
    int y ;
    ...
    switch(cmd){
        case 0:
            uid = (int)luaL_checkinteger(L, 2);
            skynet_debugscene();
            break;
        case 1:
            uid = (int)luaL_checkinteger(L, 2);
            skynet_leavescene(uid);
            break;
        case 2:
            uid = (int)luaL_checkinteger(L, 2);
            x = (int)luaL_checkinteger(L, 3);
            y = (int)luaL_checkinteger(L, 4);
            skynet_enterscene(uid, x, y);
            break;
        case 3:
            uid = (int)luaL_checkinteger(L, 2);
            x = (int)luaL_checkinteger(L, 3);
            y = (int)luaL_checkinteger(L, 4);
            skynet_movescene(uid, x, y);
            break;
        default:
            skynet_error(NULL, "cmd= %d error", cmd);
    }
    return 1;
}
